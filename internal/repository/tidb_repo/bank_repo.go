...
func (r *bankRepository) LinkBank(ctx context.Context, userID uint, bankCode string) entity.Error {
	tx := r.db.Begin()
	defer func() {
		if r := recover(); r != nil {
			tx.Rollback()
	...
	if err := tx.Where("id = ?", userID).First(&userModel).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			...
			return entity.NewError(
				entity.ErrorNotFound, fmt.Sprintf("User with id %d not found", userID),
			)
		}
		tx.Rollback()
		return entity.NewError(entity.ErrorInternal, err.Error())
	}
	...
	if err := tx.Where("code = ?", bankCode).First(&bankModel).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			tx.Rollback()
			return entity.NewError(
				entity.ErrorNotFound, fmt.Sprintf("Bank with code %s not found", bankCode),
			)
		}
		tx.Rollback()
		return entity.NewError(entity.ErrorInternal, err.Error())
...
func (r *bankRepository) Create(ctx context.Context, bank entity.Bank) (entity.Bank, entity.Error) {
	...
	if err := r.db.Create(&bankModel).Error; err != nil {
		if dbErr, ok := err.(*mysql.MySQLError); ok {
			if dbErr.Number == 1062 {
				return entity.Bank{}, entity.NewError(
					entity.ErrorAlreadyExists, "Bank code has been used",
				)
			}
		}
